...
{
	FILE *filep;
	filep=fopen("current_contact_count.txt","r");
	if(filep==NULL)
	{
...
int contact_count=get_contact_count();
void save_contact_count()
{
	FILE *filep;
	filep=fopen("current_contact_count.txt","w");
	if(filep==NULL)
	{
		printf("File not found\n");
		return;
	}
	fprintf(filep,"%d",contact_count);
	fclose(filep);
}
